...
class CapacityManager(object):
    ...
    def __init__(self, iterations=10,
        ...
        if cluster_changes is None or cluster_config is None:
            self.cluster_changes, self.cluster_config = \
                self.calculate_cluster_changes(def_file)
        else:
    ...
    def update_cluster(self):
        for iteration in range(0, self.iterations):
            ...
            if done:
                break
            print "Iteration " + str(iteration) + ". Sleeping for 60 seconds"
            time.sleep(60)
...
def calculate_cluster_changes(self, def_file):
    try:
        ...
        reconciled = {}
        _calculate_changes(reconciled, additions)
        _calculate_changes(reconciled, removals, False)
        return reconciled, ring_def
    except Exception as e:
        print "Error getting device info", e
        sys.exit(-1)

def _calculate_changes(destination, source, add_operation=True):
    for builder in RING_TYPES:
